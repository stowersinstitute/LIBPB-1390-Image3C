...
mycolnames <- as.character(colnames(exprs(myflowset[[1]]))) #get all channel names
autoplot(myflowset[[1]]) #plot histograms for all params from a sample file

#generate clustering heatmap of feature correlation from a frame in the set
myframe <- myflowset[[1]] #pull out a single flow frame, should be a fully stained sample, not a FMO etc, something with a full spread of data
...
myframe <- myflowset[[1]] #pull out a single flow frame
mat <- exprs(myframe) #extract matrix
cor_mat <- cor(mat, method = "spearman") #get correlation matrix
pheatmap(cor_mat) #plot it using clustering heatmaps
...
#####
mycolnames <-  colnames(myflowset[[1]]) #get new list of parm names
autoplot(myflowset[[1]]) #plot all histograms
ChnlsToTrans <- mycolnames[c(15:18)] #which parms to transform, usually want to do all 'intensity' parameters for fluorescence channels
translist <- estimateLogicle(myflowset[[1]], ChnlsToTrans) #estimate logicle transform from data
...
autoplot(myflowsetTrans[[1]]) #plot histograms again after transforming data
