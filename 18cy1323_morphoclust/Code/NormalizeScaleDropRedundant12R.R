...
#save histogram plot for all params to file
pdf(file = "HistogramsAll.pdf", width = 10, height = 5, onefile = FALSE)
autoplot(myflowset[[4]]) #plot histograms for an example file
dev.off()
graphics.off()
...
#save heatmap of all features correlation
pdf(file = "FeatureCorrelationAll.pdf", width = 10, height = 10, onefile = FALSE)
...
frmTrim <- DaMiR.FReduct(myfrm, th.corr = 0.85, type = "spearman") #removes features with Cor values > 0.85
...
#save heatmap of all features correlation
pdf(file = "FeatureCorrelationTrimmed.pdf", width = 10, height = 10, onefile = FALSE)
pheatmap(cor_mat) #plot it using clustering heatmaps
dev.off()
graphics.off()
...
pdf(file = "HeatmapSamplesByFeatureMeans.pdf", width = 10, height = 7, onefile = FALSE)
...
#defaul color scale
pdf(file = "SampleCorrelationByFeatureMeans.pdf", width = 10, height = 10, onefile = FALSE)
...
meansCor <- cor(t(meansMat), method = "spearman") #correlation on transposted means matrix
tokeep <- findCorrelation(meansCor, cutoff = 0.98, names = FALSE) #list of samples with mean cor >0.9
tokeepnames <- findCorrelation(meansCor, cutoff = 0.98, names = TRUE) #same but return names not index
...
pdf(file = "ClusteringHeatmapOutliersGone.pdf", width = 10, height = 6, onefile = FALSE)
pheatmap(meansMat, scale = "column", annotation_row = rowAnnot) #make clustering heatmap of feature means per sample
dev.off()
graphics.off()

pdf(file = "CorrelationSampleOutliersGone.pdf", width = 10, height = 10, onefile = FALSE)
pheatmap(cor(t(meansMat)), method = "spearman")
dev.off()
graphics.off()
...
pdf(file = "HistogramsFeaturesTrimmed.pdf", width = 10, height = 8, onefile = FALSE)
...
#autoplot(myflowsetTrans, mycolnames[18], mycolnames[21]) + geom_hex(bins=40) #have a look at some data
#autoplot(myflowsetTrans[[4]])

pdf(file = "IntensityDraq5HistogramsOrig.pdf", width = 4, height = 10, onefile = FALSE)
...
flowsetScaled <- scaleDNA(myflowsetTrans, 18) #pass flowset and parameter number of intensity-DNA

pdf(file = "IntensityDraq5HistogramsScaled.pdf", width = 4, height = 10, onefile = FALSE)
...
tokeep <- myflowset@phenoData@data$name #get sample names in flowset
tokeep <- tokeep[c(1:2,4:19)] #list of samples to kepe
...
pdf(file = "IntensityDraq5Normalized.pdf", width = 4, height = 10, onefile = FALSE)
